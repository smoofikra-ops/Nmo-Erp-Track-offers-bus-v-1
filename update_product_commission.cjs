...
// 1. Add imports
code = code.replace(
  "import { Plus, Minus, Search, Save, ShieldCheck, Printer, ArrowLeft } from 'lucide-react';",
...
// 6. Update Validation
code = code.replace(
  "const canProceedToSummary = selectedEmployeeId && totalProductsCount > 0;",
  "const canProceedToSummary = selectedEmployeeId && totalProductsCount > 0 && requiredItems.every(i => i.description.trim() !== '' && Number(i.amount) >= 0) && paymentItems.every(i => Number(i.amount) >= 0 && (i.method !== 'OTHER' || (i.description && i.description.trim() !== '')));"
);
