...
const startIndex = code.indexOf('function FormattedAmountInput');
const endIndex = code.indexOf('interface DiscountListProps');

if (startIndex !== -1 && endIndex !== -1) {
  let before = code.substring(0, startIndex);
  let after = code.substring(endIndex);

  let newCode = `function FormattedAmountInput({ value, onChange, placeholder, className }: { value: number, onChange: (val: number) => void, placeholder: string, className: string }) {
...
`;
  ...
  fs.writeFileSync('src/components/commissions/FinancialLists.tsx', before + newCode + after);
}
